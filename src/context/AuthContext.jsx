...
export function AuthProvider({ children }) {
  ...
  const login = () => {
    return Promise.reject(
      new Error("Use Clerk sign-in UI for frontend authentication")
    );
  };

  const register = () => {
    return Promise.reject(
      new Error("Use Clerk sign-up UI for frontend authentication")
    );
  };
  ...
  const updateUserProfile = async (updates) => {
    ...
    const query = clerkId
      ? supabase.from("users").update(updates).eq("clerk_id", clerkId)
